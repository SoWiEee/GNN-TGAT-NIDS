...
async def download_report(session_id: UUID, format: str = Query(default="pdf", pattern="^(pdf|html)$")):
    sdir = SESSIONS_DIR / str(session_id)
    path = sdir / f"report.{format}"
    if not path.exists():
        raise HTTPException(404, detail=f"Report ({format}) not generated yet. Call POST /report first.")
...
async def get_metrics():
    ...
    if not METRICS_PATH.exists():
        # Return placeholder when not yet computed
        return {
            "graphsage": {"clean_f1": None, "dr_under_cpgd_eps01": None, "delta_f1_after_adv_training": None},
            "gat": {"clean_f1": None, "dr_under_cpgd_eps01": None, "delta_f1_after_adv_training": None},
        }
